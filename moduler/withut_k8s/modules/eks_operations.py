...
class EKSOperations:
    ...
    def get_latest_amis(self, version: str) -> Tuple[Dict[str, Dict[str, str]], str]:
        ...
        try:
            ...
            for os_path in self.OS_PATHS:
                ...
                try:
                    ...
                    if error_code == 'ParameterNotFound':
                        os_name = os_path.split('/')[0].replace('-', ' ').title()
                        unavailable_os.append(os_name)
                    else:
                        unavailable_os.append(f"{os_path}: {error_code}")
            
            if not os_amis:
                error_summary = f"No AMI data found for version {version}. Unavailable: {', '.join(unavailable_os)}"
                Logger.warning(error_summary, indent=1)
                return {}, error_summary
            
            if unavailable_os:
                Logger.info(f"OS types not available for v{version}: {', '.join(unavailable_os)}", indent=2)
            ...
            for os_path, ami_data in os_amis.items():
                os_name = os_path.split('/')[0].replace('-', ' ').title()
                Logger.info(f"  • {os_name}: {ami_data['ami_id']} (published: {ami_data['publication_date']})", indent=1)
            return os_amis, ""
        except ClientError as e:
